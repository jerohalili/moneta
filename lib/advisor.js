...
import { birExplainGraduatedTable, birApplyGraduatedTable } from './freelancerTax'
import { birComputePenalties } from './penalties'

// Rule-based freelancer tips, each traceable to a BIR rule (NIRC Secs. 24(A), 116).
// Conservative by design: general info, not CPA advice.
...
// Advice plan for the Dashboard: ranked peso-valued actions + line-by-line walkthroughs.
// Every action cites its rule (NIRC/RR); impacts are modeled estimates.
...
function buildSlices(taxableIncome) {
  return birExplainGraduatedTable(taxableIncome).slices.map((s) => ({
...
// buildAdvicePlan input: profile flags + annual totals + prior calc results (see call sites).
// Outputs ranked actions and walkthroughs.
export function buildAdvicePlan({
  ...
  if (hasBusinessIncome && comparison) {
    ...
    if (graduatedItemized && best.method === 'graduated-itemized' && graduatedOsd) {
      actions.push({
        ...
        detail:
          ...
          'accounts — the ledger above is your running evidence file.',
        href: null,
      })
    }
    ...
    if (best.taxableIncome > 0 && best.method !== '8-percent') {
      ...
      const headroomSaving =
        birExplainGraduatedTable(best.taxableIncome).total -
        birExplainGraduatedTable(Math.max(0, best.taxableIncome - EXAMPLE_EXTRA)).total
      if (headroomSaving > 0) {
        actions.push({
          id: 'expense-headroom',
          tag: 'deductions',
          ...
          detail:
            best.method === 'graduated-osd'
              ? `You're currently on the ${RATES.OSD_RATE * 100}% standard deduction, which ignores actual expenses. ` +
                ...
                'taxable income. Log them in the ledger above through the year.'
              : 'Every legitimate, ordinary-and-necessary business expense documented with a receipt reduces your ' +
                ...
                'professional fees all count. Log them in the ledger above as you go — reconstructing receipts in ' +
  ...
  if (hasEmployeeIncome && employeeResult) {
    ...
    if (isSmwe) {
      const counterfactualTax = birApplyGraduatedTable(Math.max(0, grossCompensation - mandatoryContributions))
  ...
  if (estimatedAnnualTax > 0) {
    const penaltyExample = birComputePenalties({ basicTax: estimatedAnnualTax, daysLate: 30 })
