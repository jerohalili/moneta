import { PERCENTAGE_TAX_RATE, VAT_THRESHOLD, VAT_RATE } from '@/data/taxRates2026'
...
export function computeVat({ vatableSales, vatablePurchases }) {
  const outputVat = Math.max(0, vatableSales) * VAT_RATE
  const inputVat = Math.max(0, vatablePurchases) * VAT_RATE
